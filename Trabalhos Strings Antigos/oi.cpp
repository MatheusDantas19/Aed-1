...
void removePalNum(char frasePrincipal[],char temp[]){
	int n=0,i=0,j=0,p=0,cont=0,cont2=0,acum=0,acum2=0,cont3=0,acum3=0,acum4=0;
	...
	for(i=0;i<n;i++){
		...
		if((frasePrincipal[i]==32) || (i==n-1) || (frasePrincipal[i]==34)
			...
			if(cont>=1 && acum>=0){
				...
				}else if(cont2>0 && cont3>0 && acum>0){
					for(j=acum4;j<i+1;j++){
						temp[p]=frasePrincipal[j];
						p++;
					}
...
void fechaAspas(char frase[],char aux[]){
...
void fechaParenteses(char frase[],char aux[]){
...
int main(){
    ...
    removerRepetidos(temp,frasePrincipal);
   	fechaAspas(frasePrincipal,temp);
	fechaParenteses(frasePrincipal,temp);
